...
type userAuthMidware struct {
	...
	logger  *log.Logger
	website *hybridWebsite
}

func (u *userAuthMidware) ServeHTTP(w http.ResponseWriter, r *http.Request, next http.HandlerFunc) {
	// Change this to kite.com / www.kite.com when going live
	if r.Host == "test.kite.com" {
		u.website.ServeHTTP(w, r)
		return
	}
	...
	// Bypass authentication if path is a web sandbox
...
func (u *userAuthMidware) log(w http.ResponseWriter, r *http.Request, start time.Time) {
	...
	case negroni.ResponseWriter:
		...
		u.logger.Println(requestIP, "->", selectedSite, fmt.Sprintf("(%s)", userID), r.Method, url, rw.Status(), rw.Size(), time.Now().Sub(start), userAgent)
	case http.ResponseWriter:
		// Log what we can for http.ResponseWriter
		u.logger.Println(requestIP, "->", selectedSite, fmt.Sprintf("(%s)", userID), r.Method, url, time.Now().Sub(start), userAgent)
	}
}
