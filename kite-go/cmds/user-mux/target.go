...
var (
	targetPort = strings.TrimPrefix(envutil.GetenvDefault("USER_MUX_TARGET_PORT", ":9090"), ":")
	debugPort  = strings.TrimPrefix(envutil.GetenvDefault("USER_MUX_TARGET_DEBUG_PORT", ":9091"), ":")
)
...
func (p *proxyTarget) setHealth(err error) {
	...
	if !p.health && p.healthyCount >= healthCheckTolerance {
		p.health = true
		p.healthyCount = 0
		log.Println("target", p.target, "became healthy :)")
	}
	if p.health && p.unhealthyCount >= healthCheckTolerance {
		p.health = false
		p.unhealthyCount = 0
		log.Println("target", p.target, "became unhealthy :(")
	}

	log.Printf("set health called on %s: health: %t, healthyCount: %d, unhealthyCount: %d : %v",
...
func (p *proxyTarget) startHealthCheckLoop() {
	checkLoop := func() {
		log.Println("starting health check for target", p.target.String())
		...
		for {
			...
			case <-p.healthContext.Done():
				log.Println("stopping health check for target", p.target.String())
				return
			}
		}
	}
