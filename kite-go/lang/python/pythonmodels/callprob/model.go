...
// thresholds
func NewBaseModel(dir string, partialCallModel bool) (*Model, error) {
	modelName := "full_call_prob_model.frozen.pb"
	if partialCallModel {
		modelName = "truncated_call_prob_model.frozen.pb"
	}
...
func NewModel(dir string, partialCall bool) (*Model, error) {
	model, err := NewBaseModel(dir, partialCall)
	if err != nil {
		return nil, err
	}
	...
	if partialCall {
		thresholds = thresholdSet.PartialCall
	}
