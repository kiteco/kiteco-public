...
// StopWords returns a map containing common english stop words
// as the key values.
func StopWords() map[string]interface{} {
	words := make(map[string]interface{})
	scanner := bufio.NewScanner(bytes.NewBuffer(englishStopWords))
	...
	for scanner.Scan() {
		...
		if !strings.Contains(line, "|") {
			...
			if strings.Trim(line[:idx], " ") != "" {
				word = strings.Trim(strings.Split(line, "|")[0], " ")
			}
		}
		if word == "" {
			continue
