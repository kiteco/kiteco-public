...
class Feed(NamedTuple):
    ...
    def comp_feature_vector(cls, comp_features: RawCompFeatures) -> List[float]:
        return [float(comp_features.num_args),
                float(comp_features.pattern_freq),
                float(comp_features.score),
                float(comp_features.type_match_score),
                float(comp_features.types_violated),
                float(comp_features.pattern_match),
                float(comp_features.effective_args),
                float(comp_features.subtok_match_score),
                float(comp_features.subtoks_violated),
                float(comp_features.placeholder_count),
                float(comp_features.placeholder_scope_ratio),
                ]
    ...
    def from_samples(cls, batch_samples: List[RawSample]):
        ...
        for i, sample in enumerate(batch_samples):
            ...
            comp_features.append(comp_vect(sample.features.comp))
            sample_ids.append(i)
