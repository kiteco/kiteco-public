	...
	for {
		...
		case <-ticker.C:
			...
			if trainSampleWriter.StepsWritten() >= targetSteps &&
				...
				// Log filtered rate
				f := atomic.LoadInt64(&encodingLengthFilter)
				t := atomic.LoadInt64(&encodingLengthTotal)
				log.Printf("encoding length (max %d) filtered %d/%d, %.02f", maxEncodingLength, f, t, float64(f)/float64(t))
				return
			}
		}
	}
...
func datagen(contextSize int, enc *lexicalv0.FileEncoder, writer *utils.SampleWriter) func(pipeline.Record) {
	...
	return func(record pipeline.Record) {
		...
		context, err := extractSample(kv.Key, bs, &m, contextSize, enc, concatByExt, minContextToConcat)
		if err != nil {
...
var (
	encodingLengthFilter int64
	encodingLengthTotal  int64
)

func extractSample(key string, content []byte, m *sync.Mutex, contextSize int, enc *lexicalv0.FileEncoder, concatByExt map[string][]int, minContextToConcat int) ([]int, error) {
	...
	encoded, filtered := encode(enc, key, toks, idx, contextSize)

	atomic.AddInt64(&encodingLengthTotal, 1)
	if filtered {
		atomic.AddInt64(&encodingLengthFilter, 1)
	}
	...
	if len(encoded) < minContextToConcat {
		return nil, nil
	}
...
func encode(enc *lexicalv0.FileEncoder, filename string, toks []lexer.Token, randIdx, contextSize int) ([]int, bool) {
	var suffix []int
	for i := randIdx; i < len(toks); i++ {
		nextToks := enc.EncodeTokens([]lexer.Token{toks[i]})
		if maxEncodingLength > 0 && len(nextToks) > maxEncodingLength {
			return nil, true
		}
		suffix = append(suffix, nextToks...)
		if len(suffix) >= contextSize {
			return enc.PrepareBeforeContext(suffix, contextSize, filename), false
		}
	}
	...
	for i := randIdx - 1; i > -1; i-- {
		...
		prefix = append(nextToks, prefix...)
		if len(prefix)+len(suffix) >= contextSize {
			return enc.PrepareBeforeContext(append(prefix, suffix...), contextSize, filename), false
		}
	}
