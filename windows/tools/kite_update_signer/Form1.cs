    ...
    public partial class Form1 : Form {
        ...
        private void Form1_Load(object sender, EventArgs e) {
            ...
            if (curPath.Length > 0) {
                ...
                txtUpdateExecutableFilePath.Text = Path.Combine(curPath, @"builds\" + targetBuild + string.Format(@"\KiteUpdater{0}.exe", targetBuild));
                txtOutFilePath.Text = Path.Combine(Path.GetDirectoryName(txtUpdateExecutableFilePath.Text),
                    "KiteUpdateInfo.xml");
                txtDownloadUrl.Text = String.Format("https://windows.kite.com/windows/{0}/KiteUpdater.exe", targetBuild);
            } else {
            ...
            if(Environment.GetCommandLineArgs().Length > 1 && Environment.GetCommandLineArgs()[1] == "--sign-with-test") {
                ...
                txtOutFilePath.Text = Path.Combine(Path.GetDirectoryName(txtUpdateExecutableFilePath.Text),
                    "KiteUpdateInfo-SignedWithTESTKey.xml");
                btnUseTestKey_Click(null, EventArgs.Empty);
