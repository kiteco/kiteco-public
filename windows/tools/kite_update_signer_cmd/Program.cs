...
namespace KiteUpdateSignerCmd {
    ...
    static class Program {
        ...
        private static BuildParams buildParams()
        {
            ...
            bp.ExecutablePath = Path.Combine(curPath, @"builds\" + targetBuild + string.Format(@"\KiteUpdater{0}.exe", targetBuild));
            bp.XMLPath = Path.Combine(Path.GetDirectoryName(bp.ExecutablePath),
                "KiteUpdateInfo.xml");
            bp.DownloadURL = String.Format("https://windows.kite.com/windows/{0}/KiteUpdater.exe", targetBuild);
            return bp;
        }
